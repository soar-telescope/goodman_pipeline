...
image_list = glob.glob('*comp*fits')
for image in image_list:
    # image = '20161125_multiobject_n2_s1-fwhm.fits'
    data = fits.getdata(image)

    sample = np.median(data[700:750,:], axis=0)

    # foo = np.where(np.abs(data > data.min() + 0.75 * data.max()), data, 0)
    foo = np.where(np.abs(sample > sample.min() + 0.1 * sample.max()), sample, 0)
    ...
    if maxima != []:
        ...
        for val in maxima:
            if voigt_init is None:
                voigt_init = models.Gaussian1D(amplitude=sample[val], mean=val)
            else:
                voigt_init += models.Gaussian1D(amplitude=sample[val], mean=val)
            plt.axvline(val, color='r', alpha=0.5)
